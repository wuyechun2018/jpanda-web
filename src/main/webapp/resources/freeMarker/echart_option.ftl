 ...
 <#list mapSeries as chartSeries>
  ,{  name: '${chartSeries.seriesName}',
    ...
    markLine: {
        ...
        data : [
        	<#list chartSeries.dataList as mapdata>
	        	 <#if !mapdata_has_next> 
					 [{name:'${mapdata.srcObjName}'}, {name:'${mapdata.tarObjName}'}]
				 <#else>
					  [{name:'${mapdata.srcObjName}'}, {name:'${mapdata.tarObjName}'}],
			    </#if> 	
            </#list>
