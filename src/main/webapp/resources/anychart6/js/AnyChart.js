 ...
 * 
 * @static
 * @class Browser Brower information
 */
 ...
 * @param {String}
 *            [swfPath] path to chart swf file
 * @param {String}
 *            [preloaderSWFPath] path to chart preloader swf file
 */
...
AnyChart.prototype = {
	...
	/**
	 * Text to be shown when AnyChart gets no data source (neither XMLFile nor
	 * XMLText is set to chart)
	 * 
	 * @field {String}
	 */
	...
	/**
	 * Sets the Window Mode property of the SWF file for transparency, layering,
	 * and positioning in the browser. Valid values of wmode are window, opaque,
	 * and transparent. Set to <code>window</code> to play the SWF in its own
	 * rectangular window on a web page. Set to <code>opaque</code> to hide
	 * everything on the page behind it. Set to <code>transparent</code> so
	 * that the background of the HTML page shows through all transparent
	 * portions of the SWF file.
	 * 
	 * @field {String}
	 */
	wMode : "opaque",
	 ...
	 * if target not specified - Directly write to the current window else if
	 * target is String - Write to element in the current window by its id else
	 * write to element in the current window by its reference
	 * 
	 * @method
	 * @param {Object}
	 *            [target]
	...
	write : function() {
		...
		if (arguments[0] == undefined) {
			...
			if (!Browser.isIE
					&& (!Browser.isSafari && !Browser.isOpera && target instanceof Window)) {
				this._writeToWindow(target, htmlCode);
			} else if (typeof(target) == 'string'
					|| (!Browser.isSafari && target instanceof String)) {
	...
	_rebuildExternalInterfaceFunction : function(obj, functionName) {
		eval('obj[functionName] = function(){return eval(this.CallFunction("<invoke name=\\"'
				+ functionName
	...
	_getMoviePath : function() {
		return this.preloaderSWFFile != null
				? this.preloaderSWFFile
				: this.swfFile;
	},
	...
	_getEmbedHTML : function() {
		var source = '<embed type="application/x-shockwave-flash" pluginspage="'
				+ this._protocol + '://www.adobe.com/go/getflashplayer" ';
	...
	_initIEPrinting : function() {
		...
		window.attachEvent("onbeforeprint", function(e) {
			...
			obj.width = (obj.getWidth != undefined)
					? obj.getWidth()
					: obj.width;
			obj.height = (obj.getHeight != undefined)
					? obj.getHeight()
					: obj.height;
	...
	_createPrintCSS : function() {
		...
		if (this.flashObject != null && this.flashObject.getWidth != undefined
	...
	_resizeChart : function() {
		if (AnyChart.useBrowserResize
				&& this.flashObject.ResizeChart != undefined)
	...
	setData : function(data) {
		if (typeof(data) == 'string'
				|| (!Browser.isSafari && data instanceof String)) {
	 ...
	 * 
	 * dashboard: setLoading(viewId, messageText)
	 * 
	 * global: setLoading(messageText)
	 * 
	 * @param {String}
	 *            messageTextOrViewId message text or view id
	 * @param {String}
	 *            messageText message text
	 */
	setLoading : function() {
		if (this.flashObject == null || this.flashObject.SetLoading == null)
			return;
		switch (arguments.length) {
			case 1 :
	...
	_getFlashPlayerVersion : function() {
		...
		if (navigator.plugins != null && navigator.mimeTypes.length > 0) {
			...
			if (flashPlugin != null && flashPlugin.description != null) {
				var versionInfo = flashPlugin.description.replace(
		...
		if (activeX == null)
			return null;
		var versionInfo = activeX.GetVariable("$version").split(" ")[1]
				.split(",");
		return {
			major : versionInfo[0],
			minor : versionInfo[1],
 ...
 * @field {String}
 */
AnyChart.xmlLoadingText = "正在加载数据,请稍候！";
 ...
 * @field {String}
 */
AnyChart.noDataText = "无数据,请点击查询按钮！";
 ...
 * @field {String}
 */
AnyChart.waitingForDataText = "正在加载数据,请稍候！";
